...
function binary_search_iter(a, element, start, end)
{
    var mid, m;
    while(true)
    {
...
function insert(a, element, index, last_insert_index)
{
    var t;
...
function balance(a, num_spaces, total_inserted, last_insert_index)
{
...
Sort.LibrarySort = function( a, eps ) {
    ...
    for(ai=0,i=0; i<copy_len; i++) if ( null!=copy[i] ) a[ ai++ ] = copy[ i ]; 
    return a;
};
